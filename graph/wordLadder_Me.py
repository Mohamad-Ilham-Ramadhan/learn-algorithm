...
def ladderLength(beginWord, endWord, wordList):
    # check if there is the endWord in wordList
    ...
    if len(graph[beginWord]) == 0: return 0
    ...
    print('after all connection graph', graph)
    if endWord not in graph: return 0

# bfs: True but time limit exceeded
from collections import deque
def solution2(beginWord, endWord, wordList):
    if endWord not in wordList: return 0
    ...
    while len(q): 
        w1, c = q.popleft()
        visited.add(w1)
        # check current word with the endWord
        if wordMatch(w1, endWord): return c + 1

        # add adjacent word to the queue
        for w2 in wordList:
            if w2 == w1 or w2 in visited: continue
            if wordMatch(w1, w2): 
                q.append((w2, c+1))
                # hashmap[w1].add((w2))
    return 0

# bfs: not tracking the visited word but remove from the wordList, to make remove operation efficient I uses hashset, convert wordList to hashset
def solution3(beginWord, endWord, wordList):
